...
class _UserDetailScreenState extends State<UserDetailScreen> {
        ...
        child: CustomScrollView(
          ...
          slivers: [
            SliverAppBar(
              ...
              title: Text(
                _isLoading ? 'User Details' : (_user?['email'] ?? 'User Details'),
            ...
            if (_isLoading)
              ...
              SliverFillRemaining(
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.error_outline, size: 48, color: Colors.white.withOpacity(0.1)),
                  const SizedBox(height: 16),
                  const Text('Failed to load user', style: TextStyle(color: _textDim)),
                ]),
              )
            else
              SliverPadding(
  ...
  Widget _buildHeader() {
    ...
    final initial = user['email'].toString().substring(0, 1).toUpperCase();
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: (isAdmin ? _primary : _blue).withOpacity(0.25), width: 1.5),
            ),
            child: Center(
              child: Text(
                initial,
                style: TextStyle(color: isAdmin ? _primary : _blue, fontSize: 26, fontWeight: FontWeight.bold),
              ),
            ),
          ),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                Text(userId, style: const TextStyle(color: _textDim, fontSize: 11)),
              ]),
            ]),
          ),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            const Text('BALANCE', style: TextStyle(color: _textDim, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
            const SizedBox(height: 4),
            Text(
              NumberFormat('#,##0.00').format(user['balance'] ?? 0),
              style: GoogleFonts.outfit(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold),
            ),
            Text('USDT', style: GoogleFonts.outfit(color: _primary, fontSize: 12, fontWeight: FontWeight.bold)),
          ]),
        ],
      ),
    );
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (txs.isEmpty)
        Container(
          ...
          child: Column(children: [
            Icon(Icons.show_chart, size: 44, color: Colors.white.withOpacity(0.07)),
            const SizedBox(height: 14),
            const Text('No transactions yet', style: TextStyle(color: _textDim, fontSize: 14)),
          ]),
        )
      else
  ...
  Widget _buildTxTile(Map<String, dynamic> tx) {
    ...
    if (status == 'COMPLETED') { statusColor = _primary; statusIcon = Icons.check_circle_outline; }
    else if (status == 'PENDING') { statusColor = _blue; statusIcon = Icons.access_time; }
    else { statusColor = _danger; statusIcon = Icons.cancel_outlined; }
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Icon(isDeposit ? Icons.arrow_downward : Icons.arrow_upward, size: 18, color: isDeposit ? _primary : Colors.white),
          ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
              tx['type'].toString()[0] + tx['type'].toString().substring(1).toLowerCase(),
              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 15),
            ),
            const SizedBox(height: 3),
            Text(
              DateFormat('MMM dd, yyyy • HH:mm').format(DateTime.parse(tx['createdAt'])),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text(
              ...
              style: GoogleFonts.outfit(color: isDeposit ? _primary : Colors.white, fontWeight: FontWeight.bold, fontSize: 14),
            ),
            const SizedBox(height: 6),
            Container(
  ...
  Future<void> _updateTxStatus(String id, String status) async {
    try {
      final res = await _api.patchRequest('/wallet/transactions/$id/status', {'status': status});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 14),
        Expanded(child: Text(label, style: const TextStyle(color: _textDim, fontSize: 14))),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: (isAdmin ? _primary : Colors.white).withOpacity(0.20)),
      ),
      child: Text(
        role,
        style: TextStyle(color: isAdmin ? _primary : Colors.white, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1),
      ),
    );
  }
}
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _DetailRow(
                        label: 'USER',
                        value: tx['user']?['email'] ?? 'Current User',
                      ),
                      const SizedBox(height: 16),
                      _DetailRow(
                        label: 'AMOUNT',
